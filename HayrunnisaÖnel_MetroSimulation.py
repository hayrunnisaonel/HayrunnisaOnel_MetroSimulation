...
class MetroAgi:
    ...
    def en_az_aktarma_bul(self, baslangic_id: str, hedef_id: str) -> Optional[List[Istasyon]]:
        ...
        ziyaret_edildi = set()
        kuyruk = deque()
        kuyruk.append((baslangic, [baslangic], 0))  # (istasyon, yol, aktarma)

        while kuyruk:
            simdiki, yol, aktarma = kuyruk.popleft()
            if simdiki.idx == hedef.idx:
                return yol
            ziyaret_edildi.add((simdiki.idx, simdiki.hat))

            for komsu, _ in simdiki.komsular:
                yeni_aktarma = aktarma + (1 if komsu.hat != simdiki.hat else 0)
                if (komsu.idx, komsu.hat) not in ziyaret_edildi:
                    kuyruk.append((komsu, yol + [komsu], yeni_aktarma))
                    ziyaret_edildi.add((komsu.idx, komsu.hat))
    ...
    def en_hizli_rota_bul(self, baslangic_id: str, hedef_id: str) -> Optional[Tuple[List[Istasyon], int]]:
        ...
        while pq:
            sure, _, simdiki, yol = heapq.heappop(pq)

            if simdiki.idx == hedef.idx:
                return yol, sure

            for komsu, komsu_sure in simdiki.komsular:
                toplam_sure = sure + komsu_sure
                if komsu.idx not in en_kisa_sure or toplam_sure < en_kisa_sure[komsu.idx]:
                    en_kisa_sure[komsu.idx] = toplam_sure
                    heapq.heappush(pq, (toplam_sure, id(komsu), komsu, yol + [komsu]))
